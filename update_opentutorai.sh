...
cp temp-opentutorai/backend/requirements.txt . # TODO mettre à jour le fichier requirements.txt avec uniquepent les dépendances du backend TutorAI
rm -rf temp-opentutorai
pip install -r requirements.txt
git add backend/open_tutorai requirements.txt
git commit -m "Updating TutorAI backend"
git push
