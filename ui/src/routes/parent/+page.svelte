<script lang="ts">
	...
	import MessagesInbox from '$lib/components/parent/MessagesInbox.svelte';
	import TeachersList from '$lib/components/parent/TeachersList.svelte';
	import ConversationView from '$lib/components/parent/ConversationView.svelte';
	import AnnouncementsList from '$lib/components/parent/AnnouncementsList.svelte';
	...
	let directConv: { teacherId: string; studentId: string; teacherName: string; studentName: string } | null = null;
	...
	const handleMessageTeacher = (e: CustomEvent) => {
		const { teacherId, studentId, teacherName, studentName } = e.detail;
		directConv = { teacherId, studentId, teacherName, studentName };
		activeSection = 'messages';
	};

	const handleSectionChange = (section: string) => {
		activeSection = section;
		if (section !== 'messages') directConv = null;
	};

	onMount(async () => {
		try {
			if (!$user) { goto('/auth'); return; }
			if ($user.role !== 'parent') { await goto(`/${$user.role}`); return; }
			loading = false;
		} catch (err: any) {
...
{:else if error}
	<div class="flex items-center justify-center min-h-screen bg-white dark:bg-gray-900 p-6">
		<div class="bg-white dark:bg-gray-850 rounded-xl border border-gray-100 dark:border-gray-800 shadow-md p-8 max-w-sm w-full text-center">
			...
			<h2 class="text-lg font-bold text-red-600 mb-2">{$i18n.t('Error Loading Parent Page')}</h2>
...
{:else}
	<div class="flex h-screen overflow-hidden bg-gray-50 dark:bg-gray-900">
		<DashboardSidebar
			...
			on:navigate={(e) => handleSectionChange(e.detail.section)}
		/>
			...
			<header class="bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 px-4 py-3 flex items-center gap-3 z-10">
				...
				<div class="flex-1 min-w-0">
					...
					{:else if activeSection === 'teachers'}
						<h1 class="text-base font-semibold text-gray-800 dark:text-gray-100">Enseignants</h1>
						<p class="text-xs text-gray-500 dark:text-gray-400">Contactez directement les enseignants</p>
					{:else if activeSection === 'messages'}
						<h1 class="text-base font-semibold text-gray-800 dark:text-gray-100">Messages</h1>
						<p class="text-xs text-gray-500 dark:text-gray-400">Vos échanges avec les enseignants</p>
					{:else if activeSection === 'announcements'}
						<h1 class="text-base font-semibold text-gray-800 dark:text-gray-100">Annonces</h1>
						<p class="text-xs text-gray-500 dark:text-gray-400">Annonces de vos enseignants</p>
			...
			<div class="flex-1 overflow-auto p-6 bg-gray-50 dark:bg-gray-900">
				{#if activeSection === 'dashboard'}
					<div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-6">
						...
						<div class="bg-white dark:bg-gray-850 rounded-xl border border-gray-100 dark:border-gray-800 p-6 flex flex-col justify-between min-h-[140px]">
							...
							<p class="text-xl font-bold text-gray-900 dark:text-white">Parent</p>
						</div>
						...
						<div class="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white sm:col-span-2 lg:col-span-1">
							...
							<p class="text-sm font-bold mb-3">Communiquer avec l'école</p>
							<div class="flex flex-wrap gap-2">
								<button
									...
									on:click={() => handleSectionChange('teachers')}
								>
									Enseignants →
								</button>
								<button
									class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-white/20 hover:bg-white/30 text-white rounded-lg transition"
									on:click={() => handleSectionChange('announcements')}
								>
									Annonces →
								</button>
							</div>
				...
				{:else if activeSection === 'teachers'}
					<div class="overflow-y-auto h-full">
						<TeachersList on:message={handleMessageTeacher} />
					</div>
				...
				{:else if activeSection === 'messages'}
					<div class="h-full">
						{#if directConv}
							<ConversationView
								conversationId=""
								receiverId={directConv.teacherId}
								studentId={directConv.studentId}
								displayName={directConv.teacherName}
								studentName={directConv.studentName}
								on:conversationCreated={() => { directConv = null; }}
							/>
						{:else}
							<MessagesInbox bind:totalUnread={unreadCount} />
						{/if}
					</div>
