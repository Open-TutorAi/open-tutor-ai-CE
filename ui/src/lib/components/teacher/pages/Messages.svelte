<script lang="ts">
	...
	let directory: DirectoryStudent[] = [];
	let pickerQuery = '';
	$: pickerResults = directory.filter((s) =>
		(s.name ?? s.email ?? '').toLowerCase().includes(pickerQuery.toLowerCase())
	);
	...
	async function openPicker() {
		...
		if (directory.length === 0) {
			try {
				directory = await getStudentsDirectory(token());
			} catch (err) {
				directory = [];
			}
		}
	...
	async function startWith(s: DirectoryStudent) {
		showPicker = false;
		try {
			const conv = await startConversation(token(), s.student_id);
	...
	// Realtime: refresh on inbound messages; append if the thread is open.
	...
	<div class="flex items-center justify-between">
		<div>
			...
			<p class="text-gray-500 dark:text-gray-400 mt-1">
				{$i18n.t('Direct messages with your students.')}
			</p>
		</div>
		<button
...
{#if showPicker}
	...
	>
		<div class="w-full max-w-md rounded-2xl bg-white dark:bg-gray-800 shadow-xl p-6">
			...
			<input
				class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-800 px-3 py-2 text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-300 mb-3"
			...
			<div class="max-h-72 overflow-y-auto divide-y divide-gray-100 dark:divide-gray-700">
				...
				{:else}
					{#each pickerResults as s (s.student_id)}
						<button
							class="w-full text-left px-2 py-3 hover:bg-gray-50 dark:hover:bg-gray-700/40"
							on:click={() => startWith(s)}
						>
							<div class="text-gray-800 dark:text-white text-sm">{s.name ?? s.email ?? '—'}</div>
