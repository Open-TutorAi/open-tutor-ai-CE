<script lang="ts">
	...
	import { createInvitation, type InviteResult } from '$lib/apis/classrooms';
	...
	async function send() {
		...
		busy = true;
		try {
			const res: InviteResult = await createInvitation(token(), classId, email.trim(), role);
...
>
	<div class="w-full max-w-md rounded-2xl bg-white dark:bg-gray-800 shadow-xl p-6">
		<div class="flex items-center justify-between mb-4">
			<h2 class="text-lg font-semibold text-gray-800 dark:text-white">{$i18n.t('Invite')}</h2>
			<button class="text-gray-400 hover:text-gray-600" on:click={close} aria-label="Close"
				>✕</button
			>
		</div>
		<label class="block text-sm text-gray-600 dark:text-gray-300 mb-1" for="invite-email"
			>{$i18n.t('Email')}</label
		>
		<input
			id="invite-email"
			class="w-full rounded-lg border border-gray-300 dark:border-gray-600 bg-transparent px-3 py-2 text-gray-800 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-300"
			bind:value={email}
			placeholder="someone@mail.com"
		/>
		<div class="mt-3 text-sm text-gray-600 dark:text-gray-300">{$i18n.t('Invite as')}</div>
		<div class="flex gap-4 mt-1">
			<label class="flex items-center gap-2 text-gray-700 dark:text-gray-200"
				><input type="radio" bind:group={role} value="student" /> {$i18n.t('Student')}</label
			>
			<label class="flex items-center gap-2 text-gray-700 dark:text-gray-200"
				><input type="radio" bind:group={role} value="parent" /> {$i18n.t('Parent')}</label
			>
		</div>
		<div class="flex justify-end gap-2 mt-5">
			<button
				class="px-4 py-2 rounded-full text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700"
				on:click={close}
				disabled={busy}>{$i18n.t('Cancel')}</button
			>
			<button
				class="px-4 py-2 rounded-full bg-blue-600 text-white hover:bg-blue-700 disabled:opacity-50"
				on:click={send}
				disabled={busy}>✉ {$i18n.t('Send')}</button
			>
		</div>
