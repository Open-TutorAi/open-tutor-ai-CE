<script lang="ts">
	...
	$: filtered = searchQuery.trim()
		? conversations.filter((c) => {
				...
				return c.parent_name.toLowerCase().includes(q) || c.student_name.toLowerCase().includes(q);
			})
		: conversations;
	...
	>
		...
		<div class="px-4 pt-4 pb-3 border-b border-gray-100 dark:border-gray-800">
			<div class="flex items-center justify-between mb-3">
				<h2 class="text-sm font-semibold text-gray-700 dark:text-gray-200">Messages des parents</h2>
				{#if totalUnread > 0}
		...
		{:else if conversations.length === 0}
			<div class="flex flex-col items-center justify-center flex-1 py-16 px-6 text-center">
				...
				<p class="text-sm font-medium text-gray-800 dark:text-white mb-2">Aucun message de parent</p>
				<p class="text-sm text-gray-600 dark:text-gray-400">pour l'instant.</p>
			</div>
							...
							<div class="flex items-center gap-3">
								<div class="relative flex-shrink-0">
									<div class="w-10 h-10 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white font-semibold text-sm">
										{conv.parent_name[0]?.toUpperCase() ?? 'P'}
									</div>
		...
		{#if selectedConversation}
			...
			<div class="flex-1 min-h-0">
				<ConversationView
					...
					receiverId={selectedConversation.parent_id}
					studentId={selectedConversation.student_id}
					displayName={selectedConversation.parent_name}
