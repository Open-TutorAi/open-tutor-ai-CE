...
{#if show}
    ...
    >
        ...
        <div 
            id="main-content"
            class="relative text-center px-6 sm:px-10 lg:px-20 max-w-4xl mx-auto"
            in:fly={{ y: 20, duration: 800, delay: 400, easing: cubicOut }}
        >
            <h1 
                id="onboarding-title"
                class="text-4xl sm:text-5xl lg:text-7xl font-extrabold leading-tight mb-6 text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-purple-500"
            >
                <Marquee
                    duration={4000}
                    words={[
            ...
            <!-- Feature list - added for better value proposition -->
            <div class="mt-8 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 text-left max-w-3xl mx-auto">
                <div class="flex items-start space-x-3">
                    <span class="text-blue-400 text-2xl">✨</span>
                    <div>
                        <h3 class="font-bold text-blue-300">{$i18n.t('Personalized Learning')}</h3>
                        <p class="text-gray-300 text-sm mt-1">{$i18n.t('Adaptive content tailored to your pace')}</p>
                    </div>
                </div>
                <div class="flex items-start space-x-3">
                    <span class="text-blue-400 text-2xl">🔍</span>
                    <div>
                        <h3 class="font-bold text-blue-300">{$i18n.t('Deep Insights')}</h3>
                        <p class="text-gray-300 text-sm mt-1">{$i18n.t('Discover connections between concepts')}</p>
                    </div>
                </div>
                <div class="flex items-start space-x-3">
                    <span class="text-blue-400 text-2xl">🌐</span>
                    <div>
                        <h3 class="font-bold text-blue-300">{$i18n.t('Global Access')}</h3>
                        <p class="text-gray-300 text-sm mt-1">{$i18n.t('Learn anywhere, anytime, on any device')}</p>
                    </div>
                </div>
            </div>

            <!-- Get Started Button with enhanced accessibility and animation -->
            <div class="mt-10 sm:mt-12 flex justify-center">
                <button
                    bind:this={startButton}
                    class="relative group flex items-center space-x-2 px-6 py-4 bg-gradient-to-r from-blue-600 to-blue-500 hover:from-blue-500 hover:to-blue-400 transition-all duration-300 rounded-full text-lg font-medium text-white focus:outline-none focus:ring-4 focus:ring-blue-300 focus:ring-offset-2 focus:ring-offset-black transform hover:scale-105 active:scale-95 shadow-lg hover:shadow-xl"
                ...
                >
                    ...
                    <ArrowRightCircle class="size-6 transition-transform duration-300 group-hover:translate-x-1" />
                </button>
            </div>
            
            <!-- Privacy note for trust -->
        ...
        <!-- Decorative elements - can be removed if performance is a concern -->
        ...
        <!-- Skip button for those who want to bypass onboarding -->
        <button 
