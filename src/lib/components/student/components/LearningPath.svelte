...
<div class="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm hover:shadow-md transition-shadow duration-300">
	<div class="relative">
		...
		{#each steps as step, index}
			<div 
				class="flex items-center justify-between mb-10 relative z-10 group"
			...
			>
				...
				{#if mounted}
					<div 
						class={`flex-shrink-0 w-7 h-7 rounded-full flex items-center justify-center ml-4 transform transition-all duration-300 ${
					...
					>
						...
						{:else if step.current}
							<div class="w-2.5 h-2.5 rounded-full bg-blue-600 pulse"></div>
						{/if}
					</div>
				{/if}
