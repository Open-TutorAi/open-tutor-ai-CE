<script lang="ts">
	...
	// --- Preferences ---
	let currentTheme: string = $theme || 'system';
	...
	onMount(async () => {
		profile = {
			firstName: $user?.name ? $user.name.split(' ')[0] : 'Étudiant',
			lastName: $user?.name ? $user.name.split(' ').slice(1).join(' ') : '',
			email: $user?.email || '',
			avatar: $user?.profile_image_url || generateInitialsImage($user?.name || 'Student')
		};

		currentTheme = (localStorage.getItem('theme') as any) ?? 'system';

		const handleLanguageChange = (lng: string) => { language = lng; };
	...
	async function saveProfile() {
		...
		const updatedUser = await updateUserProfile(localStorage.token, fullName, profile.avatar).catch((error) => {
			toast.error(`${error}`);
		});
		if (updatedUser) {
			...
			toast.success($i18n.t('Profil mis à jour avec succès'));
		}
	...
	async function changePassword() {
		...
		if (newPassword.length < 8) {
			toast.error($i18n.t('Le mot de passe doit contenir au moins 8 caractères'));
			return;
		}
		...
		try {
			...
			if (response.ok) {
				toast.success($i18n.t('Mot de passe modifié'));
				oldPassword = newPassword = confirmPassword = '';
			} else {
				const error = await response.json();
				toast.error(error.detail || $i18n.t('Erreur lors du changement du mot de passe'));
			}
		} catch (err: any) {
			toast.error($i18n.t('Erreur lors du changement du mot de passe'));
		} finally {
	...
	async function savePreferences() {
		...
		try {
			// Apply language immediately and cache in localStorage
			await i18next.changeLanguage(language);
			localStorage.setItem('locale', language);

			// Save to backend (non-blocking — don't block UI if it fails)
...
<style>
    ...
    .tab-btn:hover { background: #f8faff; color: #2563eb; }
    :global(.dark) .tab-btn:hover { background: #1e2d45; color: #93c5fd; }
    .tab-btn.active { background: #eef2ff; color: #2563eb; font-weight: 600; }
    :global(.dark) .tab-btn.active { background: #1e3a8a; color: #93c5fd; }
    .tab-dot {
        width: 6px;
        height: 6px;
        background: #2563eb;
    ...
    .panel-title { font-size: 1.15rem; font-weight: 700; margin: 0 0 0.2rem; }
    ...
    :global(.dark) .field { background: #0f172a; border-color: #334155; color: #f1f5f9; }
    ...
    :global(.dark) .security-tip { background: #292100; border-color: #7c5a00; color: #fcd34d; }
    ...
    :global(.dark) .panel-footer { border-top-color: #334155; }
    ...
    .btn-save:hover { transform: translateY(-1px); box-shadow: 0 6px 18px rgba(37, 99, 235, 0.45); }
    ...
    .btn-save.green-btn:hover { box-shadow: 0 6px 18px rgba(5, 150, 105, 0.45); }
    ...
    .pref-card { display: flex; gap: 1.25rem; align-items: flex-start; padding: 0.75rem 0; }
    .pref-card-icon {
        width: 44px;
        height: 44px;
    ...
    .pref-card-content { flex: 1; }
    .pref-card-header  { margin-bottom: 0.9rem; }
    .pref-card-title   { font-size: 1rem; font-weight: 700; color: #0f172a; display: block; margin-bottom: 0.2rem; }
    :global(.dark) .pref-card-title { color: #f1f5f9; }
    .pref-card-desc    { font-size: 0.8rem; color: #64748b; margin: 0; }
    :global(.dark) .pref-card-desc { color: #94a3b8; }
    ...
    :global(.dark) .pref-separator { background: linear-gradient(90deg, transparent, #334155, transparent); }

    .lang-selector { display: flex; flex-wrap: wrap; gap: 0.6rem; }
    .lang-option {
        display: flex;
        align-items: center;
        gap: 0.5rem;
    ...
    .lang-option:hover { background: #ffffff; border-color: #2563eb; transform: translateY(-1px); box-shadow: 0 6px 10px -4px rgba(37, 99, 235, 0.15); }
    .lang-option.active { background: #2563eb; border-color: #2563eb; color: white; box-shadow: 0 8px 14px -6px rgba(37, 99, 235, 0.35); }
    :global(.dark) .lang-option { background: #1e293b; border-color: #334155; color: #cbd5e1; }
    :global(.dark) .lang-option:hover { background: #2d3a52; border-color: #60a5fa; color: #f1f5f9; }
    :global(.dark) .lang-option.active { background: #2563eb; border-color: #2563eb; color: white; }
    .lang-flag  { font-size: 1.2rem; line-height: 1; }
    .check-mark { font-size: 0.9rem; font-weight: 700; margin-left: 0.2rem; }

    .theme-selector { display: flex; flex-direction: column; gap: 0.6rem; }
    .theme-option {
        display: flex;
        align-items: center;
        gap: 1rem;
    ...
    .theme-option:hover { background: #ffffff; border-color: #2563eb; box-shadow: 0 4px 8px -2px rgba(37, 99, 235, 0.1); }
    .theme-option.active { background: #eef2ff; border-color: #2563eb; }
    :global(.dark) .theme-option { background: #1e293b; border-color: #334155; }
    :global(.dark) .theme-option:hover { background: #2d3a52; border-color: #60a5fa; }
    :global(.dark) .theme-option.active { background: #1e3a8a; border-color: #3b82f6; }
    .theme-icon-large { font-size: 1.6rem; line-height: 1; width: 32px; text-align: center; }
    .theme-label { flex: 1; font-size: 0.95rem; font-weight: 500; color: #1e293b; }
    ...
    .theme-option.active .theme-radio { border-color: #2563eb; }
    :global(.dark) .theme-radio { background: #0f172a; border-color: #475569; }
    :global(.dark) .theme-option.active .theme-radio { border-color: #3b82f6; }
    .radio-dot { width: 10px; height: 10px; background: #2563eb; border-radius: 50%; display: inline-block; }
    :global(.dark) .radio-dot { background: #3b82f6; }

    .password-wrapper { position: relative; display: flex; align-items: center; }
    .password-wrapper .field { padding-right: 2.8rem; }
    .toggle-password {
        position: absolute;
        right: 0.6rem;
        top: 50%;
    ...
    .toggle-password:hover { color: #2563eb; background: #eef2ff; }
    :global(.dark) .toggle-password { color: #64748b; }
    :global(.dark) .toggle-password:hover { color: #93c5fd; background: #1e3a8a; }
    ...
    @media (max-width: 768px) {
        ...
        .panel-header, .panel-footer, .form-stack, .pref-list-new { padding-left: 1.25rem; padding-right: 1.25rem; }
        .divider { margin-left: 1.25rem; margin-right: 1.25rem; }
        .security-tip { margin-left: 1.25rem; margin-right: 1.25rem; }
    }
