<script lang="ts">
	...
	// Render **bold** and *italic* in messages
	...
	onMount(async () => {
		const currentCourseId = $page.url.searchParams.get('course');
		if (currentCourseId) activeChannelId = currentCourseId;
	...
	async function fetchStudentRooms() {
		try {
			...
			if (!token) return;
			const res = await fetch('/api/v1/discussions/student/courses', {
	...
	async function fetchDiscussions(roomId: string) {
		...
		try {
			isLoading = true;
			const token = localStorage.getItem('token') ?? '';
			const res = await fetch(`/api/v1/discussions/rooms/${roomId}/messages`, {
				headers: { Authorization: `Bearer ${token}` }
			});
			if (res.ok) {
	...
	async function sendMessage() {
		...
		const token = localStorage.getItem('token') ?? '';
		try {
			const res = await fetch(`/api/v1/discussions/rooms/${activeChannelId}/send`, {
				method: 'POST',
				headers: {
	...
	async function saveEdit(msgId: number) {
		...
		const token = localStorage.getItem('token') ?? '';
		try {
			const res = await fetch(`/api/v1/discussions/messages/${msgId}`, {
				method: 'PUT',
	...
	async function deleteMessage(msgId: number) {
		if (!confirm($i18n.t('Voulez-vous vraiment supprimer ce message ?'))) return;
		const token = localStorage.getItem('token') ?? '';
		try {
			const res = await fetch(`/api/v1/discussions/messages/${msgId}`, {
				method: 'DELETE',
			...
			if (res.ok) await fetchDiscussions(activeChannelId);
		} catch (error) {
...
>
	...
	<main class="flex-1 flex flex-col min-h-0 bg-white dark:bg-[#030712] overflow-hidden">
			...
			<div class="flex items-center gap-2 md:gap-3 min-w-0">
				...
				<div class="min-w-0">
					...
					<p class="msg-chat-header-sub">
						👥 {activeChannel?.members_count || 1} {(activeChannel?.members_count || 1) > 1 ? $i18n.t('Membres') : $i18n.t('Membre')}
					</p>
				</div>
			</div>
		</header>
		...
		<!-- Messages list -->
		<div bind:this={messagesContainer} class="msg-area flex-1 min-h-0 overflow-y-auto">
			{#if isLoading}
				<div class="flex flex-col gap-4 animate-pulse p-6">
					{#each [1,2,3] as _}
						<div class="flex gap-4 items-start">
							<div class="w-9 h-9 rounded-full bg-slate-200 dark:bg-slate-700 flex-shrink-0"></div>
			...
			{:else}
				{#each currentMessages as m, i (m.id)}
					...
					{/if}
					{@const isOwn = m.sender_id && $user?.id && String(m.sender_id).trim() === String($user.id).trim()}
					<div class="msg-row {isOwn ? 'msg-row-own' : 'msg-row-other'} group">
						<!-- Avatar -->
						<div class="msg-avatar {m.sender_role === 'teacher' ? 'msg-avatar-own' : 'msg-avatar-other'}">
							{m.sender_name ? m.sender_name.charAt(0).toUpperCase() : '?'}
						</div>
						<!-- Body -->
						<div class="msg-body {isOwn ? 'msg-body-own' : 'msg-body-other'}">
							<div class="msg-meta {isOwn ? 'msg-meta-own' : ''}">
								...
								{#if isOwn}
									<div class="relative inline-block" on:click|stopPropagation>
										<button on:click={() => (activeMenuId = activeMenuId === m.id ? null : m.id)}
											class="msg-menu-btn opacity-0 group-hover:opacity-100">•••</button>
										{#if activeMenuId === m.id}
											<div class="msg-dropdown left-0">
		...
		<footer class="msg-footer">
			<div class="msg-input-wrap" on:click|stopPropagation>
				...
				<div class="msg-input-bottom">
					...
					<button on:click={sendMessage} disabled={!newMessage.trim()} class="msg-send-btn">
...
<style>
	...
	.msg-dropdown {
		...
		min-width: 152px; background: white;
