...
<!-- ▌2. PERFORMANCE GAUGE ▐ -->
			<div class="rounded-2xl bg-white dark:bg-gray-800 shadow-sm border border-gray-100 dark:border-gray-700 p-5 flex flex-col gap-2">
				...
				<div class="flex items-center gap-2 mb-1">
					...
					<span class="text-xs text-gray-500 dark:text-gray-400">{$i18n.t('Completion Rate')}</span>
				</div>
				<div class="flex flex-col items-center flex-1 justify-center">
					<svg viewBox="0 0 200 105" class="w-full max-w-[200px]" aria-label={$i18n.t('Performance gauge')}>
						...
						<text x="20"  y="103" font-size="7" fill="#ef4444" text-anchor="middle" font-family="sans-serif">0%</text>
						<text x="100" y="16"  font-size="7" fill="#f59e0b" text-anchor="middle" font-family="sans-serif">50%</text>
						<text x="180" y="103" font-size="7" fill="#10b981" text-anchor="middle" font-family="sans-serif">100%</text>
					</svg>
					<div class="text-center mt-2">
						<p class="text-xs text-gray-400 dark:text-gray-500">{$i18n.t('Completed Tutorials')}</p>
						<p class="text-2xl font-extrabold text-gray-800 dark:text-white tracking-tight">
							{Math.round(animPoints)}%
						</p>
					</div>
				</div>
			</div>
