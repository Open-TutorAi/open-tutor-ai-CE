<script lang="ts">
	...
	const categoryKeys = ['computer-science', 'chemistry', 'mathematics', 'other'];
	...
	$: categories = categoryKeys.map((key) => ({ id: key, label: $i18n.t(key) }));
	...
	$: showCustomCategory = courseCategory === 'other';
	$: selectedLangLabel =
		languages.find((l) => l.id === courseLanguage)?.label || $i18n.t('Select a language');
	$: selectedCatLabel = courseCategory
		? categories.find((c) => c.key === courseCategory)?.label || courseCategory
		: $i18n.t('Select a category');

	function closeLangMenu() {
		setTimeout(() => (isLangOpen = false), 150);
	}
	function closeCatMenu() {
		setTimeout(() => (isCategoryOpen = false), 150);
	}
	...
	async function generateCourse() {
		...
		if (uploadedFiles.length === 0) {
			alert($i18n.t('Please upload at least one PDF file'));
			return;
		}

		if (!selectedModel) {
			alert($i18n.t('Please select a model'));
			return;
		}
		...
		try {
			...
			lockBodyScroll(false);
			isGenerating = false;
			alert($i18n.t(e?.message ?? 'Error during generation'));
		}
	}
</script>
...
<div class="header-banner">
	<div class="header-illustration">
		<div class="icon-books">
			...
			>
				...
				<path
					d="M22 24 L27 12 L32 24 M23.5 21 H30.5"
					stroke="#378add"
					stroke-width="2"
	...
	<div class="header-text-content">
		<h1 class="page-title">{$i18n.t('My')} <span class="text-blue">{$i18n.t('Courses')}</span></h1>
		<p class="page-subtitle">{$i18n.t('Create and manage your courses with ease')}</p>
	</div>
</div>

<div class="card">
	<div class="section">
		<h2 class="section-title">{$i18n.t('General Information')}</h2>

		<div class="form-group">
			<label for="courseTitle" class="form-label">
				{$i18n.t('Course Title')} <span class="required-asterisk">*</span>
			</label>
			<input
				type="text"
				id="courseTitle"
				bind:value={courseTitle}
				placeholder={$i18n.t('Enter the course title')}
				class="form-control"
			/>
		</div>
		...
		<div class="form-row">
			<div class="form-group">
				<label for="courseLanguage" class="form-label">
					{$i18n.t('Course Language')} <span class="required-asterisk">*</span>
				</label>
				<div class="custom-select-wrapper" tabindex="0" on:blur={closeLangMenu}>
					<div
						class="form-control custom-select-trigger {isLangOpen ? 'open' : ''}"
						on:click={() => (isLangOpen = !isLangOpen)}
					>
						<span>{selectedLangLabel}</span>
						<svg
							class="chevron {isLangOpen ? 'rotate' : ''}"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
					...
					{#if isLangOpen}
						<ul class="custom-select-options">
							{#each languages as lang}
								<li
									class="custom-option {courseLanguage === lang.id ? 'selected' : ''}"
									on:click={() => {
										courseLanguage = lang.id;
										isLangOpen = false;
									}}
								>
									{lang.label}
			...
			<div class="form-group">
				<label for="courseCategory" class="form-label">
					{$i18n.t('Category')} <span class="required-asterisk">*</span>
				</label>
				<div class="custom-select-wrapper" tabindex="0" on:blur={closeCatMenu}>
					<div
						class="form-control custom-select-trigger {isCategoryOpen ? 'open' : ''}"
						on:click={() => (isCategoryOpen = !isCategoryOpen)}
					>
						<span class={!courseCategory ? 'placeholder' : ''}>{selectedCatLabel}</span>
						<svg
							class="chevron {isCategoryOpen ? 'rotate' : ''}"
							fill="none"
							stroke="currentColor"
							viewBox="0 0 24 24"
						>
					...
					{#if isCategoryOpen}
						<ul class="custom-select-options">
							{#each categories as category}
								<li
									class="custom-option {courseCategory === category.key ? 'selected' : ''}"
									on:click={() => {
										courseCategory = category.key;
				...
				{#if showCustomCategory}
					<input
						...
						placeholder={$i18n.t('Your custom category')}
		...
		<div class="form-group">
			<label class="form-label">
				{$i18n.t('Course Level')} <span class="required-asterisk">*</span>
			</label>
		...
		<div class="form-group">
			<label for="selectedModel" class="form-label">
				{$i18n.t('AI Model')} <span class="required-asterisk">*</span>
			</label>
			...
			{#if !isLoadingModels && !$models?.length && !modelLoadError}
				<p class="model-warn">
					<span style="display: inline-block; margin-right: 0.5rem;">⚠️</span>
					{$i18n.t(
						'No models available. Make sure Ollama is running and at least one model is downloaded.'
					)}
				</p>
			{/if}
		</div>
	...
	<div class="section">
		<h2 class="section-title">
			{$i18n.t('Pedagogical Objectives')} <span class="required-asterisk">*</span>
		</h2>
		<textarea
			bind:value={pedagogicalObjectives}
			placeholder={$i18n.t('Describe the pedagogical objectives and what students will learn...')}
			rows="5"
	...
	<div class="section">
		<h2 class="section-title">
			{$i18n.t('Educational Material')} <span class="required-asterisk">*</span>
		</h2>
		...
		>
			<label for="fileInput" class="dropzone-label">
				...
				<p class="dropzone-text">
					{$i18n.t('Drag and drop your PDF files here or')}
					<span class="dropzone-link">{$i18n.t('browse your folders')}</span>
				</p>
				<p class="dropzone-hint">{$i18n.t('Accepted formats: PDF (Max 500MB each)')}</p>
			</label>
			<input
				type="file"
				id="fileInput"
		...
		{#if uploadedFiles.length > 0}
			<div class="files-container">
				<h3 class="files-title">{$i18n.t('Uploaded Files')} ({uploadedFiles.length})</h3>
		...
		<div class="modal-card">
			{#if !generationSuccess}
				...
				<div class="modal-loading">
					...
					<h3 class="modal-title">{$i18n.t('Generating...')}</h3>
					<p class="modal-subtitle">{$i18n.t('Please wait while we create your course...')}</p>
			...
			{:else}
				...
				<div class="modal-success">
					...
					<h3 class="modal-title">{$i18n.t('Course generated successfully!')}</h3>
					<p class="modal-subtitle">
						{$i18n.t('Your course has been created and is now available.')}
					</p>
...
<style>
	...
	@keyframes dash {
		0% {
			stroke-dasharray: 1 213;
			stroke-dashoffset: 0;
		}
		50% {
			stroke-dasharray: 120 94;
	...
	.level-dot.primary-school {
		background: #22c55e;
	}
	.level-dot.middle-school {
		background: #0ea5e9;
	}
	.level-dot.high-school {
		background: #f59e0b;
	}
	.level-dot.university {
		background: #ef4444;
	}
