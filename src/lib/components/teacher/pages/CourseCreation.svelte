<script lang="ts">
    ...
    async function generateCourse() {
        ...
        // Sauvegarder les données du formulaire + fichiers dans le store
        ...
        try {
            ...
            // Appel preview-plan : génère sans sauvegarder en DB
            xhr.open('POST', '/api/v1/teacher/courses/preview-plan');
            ...
            // Stocker le plan en mémoire dans le store (pas de course_id)
            courseCreationData.update(d => ({
                ...d,
                generatedPlan: result.plan,
                generatedObjectives: result.objectives ?? pedagogicalObjectives
            }));
        ...
        <div class="modal-card">
            ...
            {:else}
                ...
                <div class="modal-success">
                    ...
                    <h3 class="modal-title">{$i18n.t('Plan généré avec succès !')}</h3>
                    <p class="modal-subtitle">{$i18n.t('Vérifiez et validez le plan avant de sauvegarder le cours.')}</p>
                    ...
                    <div class="modal-actions">
                        ...
                        <button class="btn-primary" on:click={() => {
                            closeModal();
                            // Navigation vers le plan editor — pas de course_id (pas encore sauvegardé)
                            goto('/teacher/courses?view=plan', {
                                state: {
                                    courseData: {
                                        ...
                                        category: showCustomCategory ? customCategory : courseCategory,
                                        custom_category: showCustomCategory ? customCategory : '',
                                        level: courseLevel,
                                        objectives: generatedCourseData.objectives ?? pedagogicalObjectives,
                                        aiObjectives: generatedCourseData.objectives ?? pedagogicalObjectives,
                                        files: uploadedFiles,
