...
<script lang="ts">
    ...
    export let courseData: {
        course_id: string | null;   // null = nouveau cours pas encore sauvegardé
        title: string;
        language: string;
        category: string;
    ...
    // ── VALIDATE : crée le cours en DB (si nouveau) ou sauvegarde (si existant) ──
    async function validateCourse() {
        ...
        try {
            ...
            if (!courseData.course_id) {
                // ── Nouveau cours : appel save-validated (créé + plan sauvegardé en une seule requête)
                const formData = new FormData();
                formData.append('title',       courseData.title);
                formData.append('language',    courseData.language);
                formData.append('category',    courseData.category);
                formData.append('custom_category', courseData.custom_category ?? '');
                formData.append('level',       courseData.level);
                formData.append('objectives',  editableObjectives);
                formData.append('model',       '');
                formData.append('plan_json',   JSON.stringify({ chapters }));
                courseData.files.forEach(f => formData.append('files', f));

                const res = await fetch('/api/v1/teacher/courses/save-validated', {
                    method: 'POST',
                ...
                // Mise à jour de course_id pour afficher le bon code dans la modal
                courseData = { ...courseData, course_id: data.course_id };
                courseCode = data.course_id;
                // Vider le store (le cours est maintenant en DB)
                courseCreationData.set({
                    courseTitle: '', courseLanguage: 'fr-FR', courseCategory: '',
                    customCategory: '', courseLevel: '', pedagogicalObjectives: '',
                    uploadedFiles: [], selectedModel: '',
                    generatedPlan: null, generatedObjectives: ''
                });
            } else {
                // ── Cours existant : mise à jour du plan uniquement
                const res = await fetch(`/api/v1/teacher/courses/${courseData.course_id}/plan`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
                    body: JSON.stringify({ chapters, objectives: editableObjectives }),
                });
                if (!res.ok) {
                    const err = await res.json().catch(() => ({}));
                    saveError = err.detail ?? $i18n.t('Erreur lors de la sauvegarde');
            ...
            saveError = e?.message ?? $i18n.t('Erreur lors de la sauvegarde');
        } finally {
            isSaving = false;
        }
    }
...
<div class="plan-editor">
    ...
    <div class="editor-actions">
        <button class="btn-outline" on:click={() => goto('/teacher/courses?view=create')}>
            {$i18n.t('Retour')}
        </button>
