<!--
  ...
  - On "Continue" in code modal → redirect to dashboard with highlight
-->
<script lang="ts">
	...
	async function savePlan() {
		...
		try {
			...
			if (!res.ok) {
				...
				saveError = err.detail ?? $i18n.t('Error while saving');
				return;
			}
			saveSuccess = true;
			setTimeout(() => (saveSuccess = false), 3000);
		} catch {
			saveError = $i18n.t('Network error');
		} finally {
			isSaving = false;
		}
	}
...
<div class="plan-editor">
	...
	<div class="hero">
		<div class="hero-content">
			<div class="hero-eyebrow">{$i18n.t('Course Plan')}</div>
		...
		<div class="hero-stats">
			<div class="hstat">
				...
				<span class="hstat-l">{$i18n.t('Chapters')}</span>
			</div>
			...
			<div class="hstat">
				...
				<span class="hstat-l">{$i18n.t('Files')}</span>
			</div>
		</div>
	</div>
	...
	{#if saveSuccess}
		<div class="banner banner-ok" in:fly={{ y: -8, duration: 200 }}>
			...
			{$i18n.t('Plan saved successfully!')}
		</div>
	{/if}
	...
	{#if chapters.length === 0}
		<div class="empty-plan-warn" in:fly={{ y: 8, duration: 250 }}>
			...
			{$i18n.t('No chapters generated — add them manually below.')}
		</div>
	{/if}
	...
	<div class="editor-main">
		...
		<div class="card">
			<div class="card-head">
				...
				<div>
					<div class="card-title">{$i18n.t('Course Structure')}</div>
					<div class="card-sub">{$i18n.t('Chapters & Sections')}</div>
				</div>
			</div>
			...
			<div class="ch-list">
					...
					<div class="ch-block" in:fly={{ y: 10, duration: 220, easing: cubicOut }}>
						<div class="ch-row" class:open={expandedChapters.has(ch.id)}>
							<button class="ch-toggle" on:click={() => toggleChapter(ch.id)}>
								...
								<svg
									class="ch-arr"
									class:rotated={expandedChapters.has(ch.id)}
									viewBox="0 0 24 24"
									fill="none"
									stroke="currentColor"
									width="13"
									...
									<div class="sec-row" in:fly={{ x: -6, duration: 200 }}>
										...
										{#if editingSection?.chapterId === ch.id && editingSection?.sectionId === sec.id}
											<div class="iedit">
												...
												/>
												<button class="iact ok" on:click={saveEditSection}
													><svg
														viewBox="0 0 24 24"
														fill="none"
														stroke="currentColor"
													...
													></button
												>
												<button class="iact no" on:click={cancelEditSection}
													><svg
														viewBox="0 0 24 24"
														fill="none"
														stroke="currentColor"
										...
										{:else}
											...
											<div class="acts sec-acts">
												<button
													class="iact ed"
													on:click={() => startEditSection(ch.id, sec)}
													title={$i18n.t('Edit')}
												>
													<svg
														viewBox="0 0 24 24"
												...
												<button
													class="iact rm"
													on:click={() => deleteSection(ch.id, sec.id)}
													title={$i18n.t('Delete')}
												>
													<svg
														viewBox="0 0 24 24"
									...
									<div class="sec-row new-sec-row" transition:fly={{ x: -6, duration: 200 }}>
										...
										<div class="iedit">
											<input
												...
												class="ifield sm"
												placeholder={$i18n.t('Section Title')}
												on:keydown={(e) => {
													if (e.key === 'Enter') addSection(ch.id);
													if (e.key === 'Escape') cancelAddSection();
												}}
											/>
											<button class="iact ok" on:click={() => addSection(ch.id)}
												><svg
													viewBox="0 0 24 24"
													fill="none"
													stroke="currentColor"
												...
												></button
											>
											<button class="iact no" on:click={cancelAddSection}
												><svg
													viewBox="0 0 24 24"
													fill="none"
													stroke="currentColor"
				...
				<div class="add-ch-row">
					<input
						...
						class="add-ch-input"
						placeholder={$i18n.t('New chapter title...')}
		...
		<div class="right-col">
			<div class="card">
				<div class="card-head">
					...
					<div>
						<div class="card-title">{$i18n.t('Pedagogical Objectives')}</div>
						<div class="card-sub">{$i18n.t('Generated by AI - modifiable')}</div>
					</div>
				</div>
				...
				<textarea
					...
					class="obj-ta"
					rows="11"
					placeholder={$i18n.t('By the end of this course, the student will be able to...')}
				></textarea>
			</div>
	...
	<div class="editor-actions">
		...
		<button class="btn-primary" on:click={validateCourse} disabled={isSaving}>
			...
			{$i18n.t('Validate Course')}
		</button>
	</div>

	<!-- ═══════════════════════════════════════════════════════
         CODE MODAL — updated: "Continue" → dashboard redirect
         ═══════════════════════════════════════════════════════ -->
	{#if showCodeModal}
		<div
			class="modal-overlay"
		...
		>
			...
			>
				<button class="modal-close" on:click={closeCodeModal} aria-label="Close">
					<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" width="20" height="20">
						<path
							stroke-linecap="round"
							stroke-linejoin="round"
				...
				<div class="code-modal-content">
					...
					<h2 class="code-modal-title">{$i18n.t('Course created successfully!')}</h2>
					<p class="code-modal-subtitle">
						{$i18n.t('Share this code with your students so they can join the course')}
					</p>

					<!-- Course title display -->
					<div class="course-title-display">
						<span class="course-title-label">{$i18n.t('Course')}</span>
						<span class="course-title-value">{courseData.title}</span>
					</div>

					<!-- Code box -->
					<div class="code-display">
						<p class="code-label">{$i18n.t('Participation code')}</p>
						<div class="code-box">
							<span class="code-text">{courseCode}</span>
							<button class="copy-btn" on:click={copyCodeToClipboard} title={$i18n.t('Copy code')}>
					...
					<p class="code-hint">
						{$i18n.t('Students will enter this code to join your course.')}
					</p>

					<!-- Continue → redirect to dashboard -->
					<button class="btn-primary-modal" on:click={closeCodeModal}>
						{$i18n.t('Continue to Dashboard')}
...
<style>
   ...
   ══════════════════════════════ */
	...
	:global(.dark) .code-modal-subtitle {
		color: #94a3b8;
	}
	...
	:global(.dark) .course-title-display {
		background: #0f172a;
		border-color: #334155;
	}
	.course-title-label {
	...
	:global(.dark) .course-title-value {
		color: #f1f5f9;
	}
