...
<script lang="ts">
	...
	// Navigation items organized by role (using keys instead of translated labels)
...
<div class="relative flex h-full">
	...
	>
		...
		<nav class="mt-2">
			{#if navItemsKeys[currentRole] && navItemsKeys[currentRole].length > 0}
				<ul>
					{#each navItemsKeys[currentRole] as item}
						<li class="mb-1 px-2">
							<button
							...
							>
								...
								{#if isSidebarOpen}
									<span class="ml-3 text-sm font-medium">{$i18n.t(item.labelKey)}</span>
								{/if}
							</button>
						</li>
					{/each}
			...
			{:else}
				<div class={`px-4 py-3 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'} text-sm italic`}>
					{isSidebarOpen ? $i18n.t('No navigation items available') : ''}
				</div>
			{/if}
		</nav>
