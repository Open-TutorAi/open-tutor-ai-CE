<script lang="ts">
	...
	onMount(() => {
		...
		const unsubscribeTheme = theme.subscribe((newTheme) => {
...
<header class={`navbar-root ${isDarkMode ? 'navbar-dark' : 'navbar-light'}`}>
		...
		<div class="ml-4">
			<h1 class={`navbar-title ${isDarkMode ? 'text-dark' : 'text-light'}`}>
				<span class="hidden sm:inline inline-flex items-center gap-2">
					{displayName
						? $i18n.t('Hello Professor') + ' ' + displayName
						: $i18n.t('Hello Professor')}
	...
	<div class="hidden md:flex items-center gap-4">
		<div class={`search-wrapper ${isSearchFocused ? 'search-focused' : 'search-idle'}`}>
			<div
				class={`search-inner ${isDarkMode ? 'search-dark' : 'search-light'} ${isSearchFocused ? 'search-ring' : ''}`}
			>
				<svg
					xmlns="http://www.w3.org/2000/svg"
					class="search-icon"
		...
		<div class="relative" bind:this={notificationRef}>
			...
			{#if showNotifications}
				<div class={`dropdown-panel ${isDarkMode ? 'dropdown-dark' : 'dropdown-light'}`}>
					<div
						class={`dropdown-header ${isDarkMode ? 'dropdown-divider-dark' : 'dropdown-divider-light'}`}
					>
					...
					<div
						class={`dropdown-footer ${isDarkMode ? 'dropdown-divider-dark' : 'dropdown-divider-light'}`}
					>
		...
		<div class="relative" id="language-dropdown-container">
			...
			{#if showLanguageMenu}
				<div
					class={`dropdown-panel dropdown-narrow ${isDarkMode ? 'dropdown-dark' : 'dropdown-light'}`}
				>
					<div
						class={`dropdown-header ${isDarkMode ? 'dropdown-divider-dark' : 'dropdown-divider-light'}`}
					>
			...
			{#if showUserDropdown}
				<div class={`dropdown-panel ${isDarkMode ? 'dropdown-dark' : 'dropdown-light'}`}>
					...
					>
						...
						<p class={`dropdown-email ${isDarkMode ? 'subtext-dark' : 'subtext-light'}`}>
							{userEmail}
						</p>
					</div>
					<div class="py-1">
					...
					<div
						class={`dropdown-footer-section ${isDarkMode ? 'dropdown-divider-dark' : 'dropdown-divider-light'}`}
					>
						<button
							on:click={() => {
	...
	<div class="flex items-center gap-3 md:hidden">
		...
		<div class="relative" bind:this={mobileMenuRef}>
			...
			{#if showMobileMenu}
				<div
					class={`dropdown-panel dropdown-mobile ${isDarkMode ? 'dropdown-dark' : 'dropdown-light'}`}
				>
					<div
						class={`dropdown-header ${isDarkMode ? 'dropdown-divider-dark' : 'dropdown-divider-light'}`}
						style="flex-direction: column; align-items: flex-start; gap: 4px;"
...
<style>
	...
	.lang-idle-light:hover {
		background-color: #f9fafb;
	}
	.lang-idle-dark {
		color: #d1d5db;
	}
	.lang-idle-dark:hover {
		background-color: #374151;
	}
