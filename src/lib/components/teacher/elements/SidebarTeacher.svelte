...
<script lang="ts">
	...
	onMount(() => {
		...
		if (pathSegments.length >= 3) {
			...
			// Mark chat nav item as active for chat pages
			// Format: /teacher/chat/ID, /teacher/chat/ID/edit, /teacher/chat/create
			if (pageFromUrl === 'chat') {
				pageFromUrl = 'chat';
			}
	...
	const navItems: NavItems = {
		...
		teacher: [
			...
			{ id: 'classrooms', label: 'My Courses', icon: Classroom },
			{ id: 'quiz', label: 'Homework & Quizzes', icon: Assignment },
			{ id: 'reports', label: 'Tracking & Reports', icon: Dashboard },
			{ id: 'chat', label: 'Discussions', icon: Message },
