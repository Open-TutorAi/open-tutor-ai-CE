...
> **Companion doc:** `security.md` covers the technical security controls; this file is the
> plain-language *how-to*. Screenshots referenced below live in
...
2. Click **Lock** on a student (or **Lock all**) to blank their TutorAI screen; **Unlock** to
   restore it. A locked student who switches away shows a **👀 Away** marker, recorded in the
   Screen activity log.
3. *Note:* locking blanks the page immediately; forcing full-screen depends on the student's
   browser and may engage on their next click — this is accountability, not an OS-level lockdown.
...
- Screen control affects the *device/session* only — it never alters a student's learning records.
